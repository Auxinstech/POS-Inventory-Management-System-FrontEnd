...
import { UpdateCategoryRequest } from "Models/category";
import React, { useEffect, useState } from "react";
import { Button, Col, FloatingLabel, Form, Row } from "react-bootstrap";
import { useDispatch } from "react-redux";
import { useNavigate, useParams } from "react-router-dom";
import ImagePicker from "../../Components/ImagePicker";
import { removeCategory, updateCategory } from "../../Redux/Ducks/homeSlice";
import { toggleLoader } from "../../Redux/Ducks/loaderSlice";
import { requestGetCategory } from "../../Redux/Sagas/Requests/home";

const initialModel: UpdateCategoryRequest = {
  id: 0,
  name: "",
  description: "",
  image: "",
...
const EditSubCategory: React.FC = () => {
  ...
  const [model, setModel] = useState<UpdateCategoryRequest>(initialModel);
  const [vat_enabled, set_vat_enabled] = useState<boolean>(true);

  const handleChange = (field: keyof UpdateCategoryRequest, value: any) => {
  ...
  const loadCategory = async () => {
    ...
    const res = await requestGetCategory(parseInt(id));
          ...
          <Row>
            ...
            <Col md="3">
              <Form.Check
                ...
                onChange={(e: any) =>
                  handleChange("eligible_collection_discount", e.target.checked)
                }
              />
            </Col>
            <Col md="3">
            ...
            <Col md="3">
              <FloatingLabel
                controlId="food_section"
                label="Food Section"
                className="mb-3"
              >
                <Form.Select
                  value={model.food_section}
                  onChange={(e) => handleChange("food_section", e.target.value)}
                >
