...
const EditVendorModal = ({
  ...
  const VendorTypes = ["retail", "wholesale", "corporate"];

  const [formData, setFormData] = useState<Partial<IVendorCreatePayload>>({
    code: "",
    name: "",
    phone: "",
    email: "",
  ...
  useEffect(() => {
    if (selectedVendor && show) {
      setFormData({
        ...
        type: selectedVendor.type || "retail",
        credit_limit: Number(selectedVendor.credit_limit) || 0,
        balance: Number(selectedVendor.balance) || 0,
        status: selectedVendor.status ?? true,
        store_id: selectedVendor.store_id || store_id,
      });
    }
  }, [selectedVendor, show, store_id]);
  ...
  const handleSubmit = async (e: any) => {
    ...
    await dispatch(
      updateVendor(
        { id: selectedVendor.id, data: formData },
        {
          onSuccess: () => {
          ...
          <Row>
            ...
            <Col md={6}>
              <Form.Group className="mb-3">
                ...
                />
                <Form.Text className="text-muted">
                  Inactive Vendors cannot make purchases
                </Form.Text>
              </Form.Group>
            </Col>
          </Row>
