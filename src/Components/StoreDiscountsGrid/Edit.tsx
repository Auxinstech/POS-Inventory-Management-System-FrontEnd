...
import { updateDiscount } from "../../Redux/Ducks/homeSlice";
import { IDiscount, IDiscountUpdatePayload } from "Models/discounts";
...
const EditDiscountModal = ({
  ...
  show: boolean;
  setShow: any;
  selectedDiscount: any; // Changed from IDiscount to any since it's mapped data
  store_id?: number;
}) => {
  ...
  const [formData, setFormData] = useState<IDiscountUpdatePayload>({
    store_id: store_id || 0,
    item_id: null,
    is_active: true,
  ...
  // Populate form when selectedDiscount changes
  useEffect(() => {
    if (selectedDiscount && show) {
      // Extract numeric values from formatted strings
      const percentageValue = extractNumericValue(selectedDiscount.percentage);
      const amountValue = extractNumericValue(selectedDiscount.amount);
      const minOrderValue = extractNumericValue(
        selectedDiscount.min_order_value,
      );
      const maxDiscountValue = extractNumericValue(
        selectedDiscount.max_discount,
      );

      setFormData({
        store_id: selectedDiscount.store_id || store_id || 0,
        item_id: selectedDiscount.item_id || null,
        is_active:
          selectedDiscount.is_active === true ||
          selectedDiscount.is_active === 1,
        discount_mode: selectedDiscount.discount_mode || "amount",
        percentage: percentageValue,
        amount: amountValue,
        min_order_value: minOrderValue,
        max_discount: maxDiscountValue,
        start_date:
          selectedDiscount.start_date && selectedDiscount.start_date !== "-"
            ? selectedDiscount.start_date
            : "",
        end_date:
          selectedDiscount.end_date && selectedDiscount.end_date !== "-"
            ? selectedDiscount.end_date
            : "",
        start_time:
          selectedDiscount.start_time && selectedDiscount.start_time !== "-"
            ? selectedDiscount.start_time
            : "00:00:00",
        end_time:
          selectedDiscount.end_time && selectedDiscount.end_time !== "-"
            ? selectedDiscount.end_time
            : "23:59:59",
        discount_type: selectedDiscount.discount_type || "delivery",
        first_time_users_only:
          selectedDiscount.first_time_users_only === true ||
          selectedDiscount.first_time_users_only === 1,
        monday:
          selectedDiscount.monday === true || selectedDiscount.monday === 1,
        tuesday:
          selectedDiscount.tuesday === true || selectedDiscount.tuesday === 1,
        wednesday:
          selectedDiscount.wednesday === true ||
          selectedDiscount.wednesday === 1,
        thursday:
          selectedDiscount.thursday === true || selectedDiscount.thursday === 1,
        friday:
          selectedDiscount.friday === true || selectedDiscount.friday === 1,
        saturday:
          selectedDiscount.saturday === true || selectedDiscount.saturday === 1,
        sunday:
          selectedDiscount.sunday === true || selectedDiscount.sunday === 1,
        postcode:
          selectedDiscount.postcode === "All"
            ? null
            : selectedDiscount.postcode,
      });
    }
  }, [selectedDiscount, show, store_id]);
  ...
  const handleSubmit = async (e: any) => {
    ...
    await dispatch(
      updateDiscount(
        { id: selectedDiscount.id, data: submitData },
        {
          onSuccess: () => {
            setShow(false);
            setFormData({
            ...
            <Col md={6}>
              <Form.Group className="mb-3">
                <Form.Check
                  ...
                  checked={formData.first_time_users_only || false}
                  onChange={handleChange}
                  disabled={loading}
                />
              </Form.Group>
                  ...
                  <Col md={3} key={day.key}>
                    <Form.Group className="mb-2">
                      <Form.Check
                        ...
                        checked={
                          (formData[
                            day.key as keyof IDiscountUpdatePayload
                          ] as boolean) || false
                        }
