...
type CatModel = UpdateCategoryRequest | CreateCategoryRequest;

interface ScheduleFormProps<T extends CatModel> {
  model: T;
  setModel: React.Dispatch<React.SetStateAction<T>>;
}

export default function ScheduleFormCategory<T extends CatModel>({
  model,
  setModel,
}: ScheduleFormProps<T>) {
