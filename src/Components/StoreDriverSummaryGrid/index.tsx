...
const columns: GridColDef<DriverSummaryItem>[] = [
  { field: "id", headerName: "ID", width: 90 },
  { field: "order_id", headerName: "Order ID", width: 150, editable: true },
  {
    field: "driver_name",
...
export default function DataGridDemo({
  ...
  const items: DriverSummaryItem[] = useMemo(() => {
    return orders
      .filter(
        (order) => order.order_type === "Delivery" && order.rider_id !== null
      )
      .map((order) => ({
        id: order.id,
        order_id: order.id,
        driver_name: `${order.rider?.name || ""} `.trim() || "-",
  ...
  // Custom hook to get filtered totals
  const useGridFilteredTotals = () => {
    ...
    const totals = useMemo(() => {
      ...
      return {
        ...
        totalAmount,
        rowCount:
          filteredRowIds.length > 0 ? filteredRowIds.length : items.length,
      };
  ...
  const handleExport = () => {
      ...
      if (apiRef.current) {
        ...
        if (filteredRowIds.length > 0) {
          dataToExport = filteredRowIds
            ...
            .filter(Boolean) as DriverSummaryItem[];
        }
      }
    } catch (error) {
    ...
    saveAs(data, `driver_reports_${fromDate}_to_${toDate}.xlsx`);
  };
